...
public class TablaDispersionDobleHashing<K extends Comparable, T> {
    ...
    // Constructor
    public TablaDispersionDobleHashing(int tamanho, Double factorCargaMax) throws Exception {
    ...
    public void agregar(K clave, T elemento) {
        ...
        while (c < this.tamanho && c != -1) {
            indiceNuevo = Math.abs(dobleHash(clave, i)) % this.tamanho;
    ...
    private int h2(int k) {
        return 1 + (k % (this.tamanho - 1));
    }

    private int dobleHash(K clave, int i) {
        // Se realiza segun el metodo explicado en el libro CLRS 3rd Edition p. 272, 273 y 274
        // h(k, i) = ( h1(k) + i*h2(k) ) mod m
        return h1(clave.hashCode()) + i * h2(clave.hashCode());
    }
